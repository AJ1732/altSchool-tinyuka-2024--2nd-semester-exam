...
export function TodoCard({
  ...
  const handleDelete = async () => await deleteTodo(id);
  ...
  return (
    <li className="hover:bg-avocado-200/50 group relative flex items-center border-b border-neutral-300/80 last:border-b-0 last:[&>*]:border-b-0">
      <div className="mx-auto grid w-full grid-cols-[1.25rem_minmax(10rem,_1fr)_1.125rem] items-center gap-y-1 px-2 py-4 lg:max-w-xl lg:grid-cols-[1.25rem_minmax(15rem,_1fr)_minmax(8rem,_12rem)_2rem] lg:gap-x-4 lg:pl-4">
        <Checkbox
          checked={completed}
          onCheckedChange={handleToggle}
          className={cn("max-lg:-mb-1", updatingTodo && "bg-avocado-300")}
        />
        ...
        >
          <Link href={`/todos/${id}`} className="hover:text-avocado-600">
            {title}
          </Link>
        </label>

        {/* DESCRIPTION */}
        <p className="line-clamp-1 text-sm font-light max-lg:col-start-2 max-lg:row-start-2 max-lg:pl-4">
          {description}
        </p>
